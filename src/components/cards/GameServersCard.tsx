...
export default function GameServersCard({title = "Game Servers", seeAllLink = "#"}: GameServersCardProps) {
      ...
      <div className="relative">
        <div
          ...
          onScroll={(e) => {
            ...
            const fadeElement = target.nextElementSibling as HTMLElement;
            if (fadeElement) {
        ...
        >
          {loading ? (
            ...
            servers.map((server) => (
              <div
                key={server.id}
                className="rounded-[25px] min-w-[350px] w-[350px] bg-cover bg-center aspect-video relative overflow-hidden bg-[#333] group flex-shrink-0 cursor-pointer transition-transform duration-300 hover:scale-[1.02]"
                style={{
            ...
            <div className="text-white/50 text-center py-10">No servers available</div>
          )}
        </div>
        <div
          className="absolute right-0 top-0 bottom-0 w-24 bg-gradient-to-l from-bg-dark to-transparent pointer-events-none transition-opacity duration-300"
          style={{opacity: 1}}
        ></div>
      </div>
    </>
