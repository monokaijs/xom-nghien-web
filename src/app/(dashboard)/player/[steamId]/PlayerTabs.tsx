...
function MatchHistoryItem({ match, playerTeam }: { match: MatchHistory; playerTeam: string }) {
  ...
  return (
    ...
    >
      {/* Gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#2b161b]/95 to-[#1a0f12]/95 group-hover:from-[#2b161b]/90 group-hover:to-[#1a0f12]/90 transition-colors" />
      ...
      <div className="relative z-10 p-4 max-md:p-3">
        ...
        <div className="flex items-center justify-between gap-3 mb-3 max-md:flex-col max-md:items-start max-md:gap-2">
          <div className="flex items-center gap-3 max-md:w-full max-md:justify-between">
            <div className={`px-3 py-1 rounded-lg text-sm font-semibold max-md:px-2 max-md:py-0.5 max-md:text-xs ${playerWon ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
              }`}>
        ...
        <div className="hidden md:flex items-center justify-between gap-4 mb-3">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className={`font-semibold truncate flex-1 text-right ${isPlayerTeam2 ? 'text-accent-primary' : 'text-white/70'}`}>
              {match.team2_name}
            </div>
          </div>
        </div>
        ...
        <div className="md:hidden space-y-1.5 mb-3">
          ...
          <div className={`flex items-center gap-3 pl-2 py-1.5 border-l-4 ${match.team1_score > match.team2_score
            ...
            <span className={`text-lg font-bold min-w-[24px] ${match.team1_score > match.team2_score ? 'text-accent-primary' : 'text-white/60'
              }`}>
              {match.team1_score}
            </span>
            <div className={`font-medium text-sm truncate flex-1 ${isPlayerTeam1 ? 'text-accent-primary' : 'text-white/70'
              }`}>
              {match.team1_name}
            </div>
          </div>
          ...
          <div className={`flex items-center gap-3 pl-2 py-1.5 border-l-4 ${match.team2_score > match.team1_score
            ...
            <span className={`text-lg font-bold min-w-[24px] ${match.team2_score > match.team1_score ? 'text-accent-primary' : 'text-white/60'
              }`}>
              {match.team2_score}
            </span>
            <div className={`font-medium text-sm truncate flex-1 ${isPlayerTeam2 ? 'text-accent-primary' : 'text-white/70'
              }`}>
              {match.team2_name}
            </div>
          </div>
