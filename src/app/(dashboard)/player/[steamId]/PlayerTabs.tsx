  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-4 max-md:p-3">
        {/* Header: Win/Loss badge, Date, Map */}
        <div className="flex items-center justify-between gap-3 mb-3 max-md:flex-col max-md:items-start max-md:gap-2">
          <div className="flex items-center gap-3 max-md:w-full max-md:justify-between">
            <div
              className={`px-3 py-1 rounded-lg text-sm font-semibold max-md:px-2 max-md:py-0.5 max-md:text-xs ${playerWon ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
              }`}>
              {playerWon ? 'Thắng' : 'Thua'}
            </div>
            <div className="text-sm text-white/50 max-md:text-xs">{formatDate(match.start_time)}</div>
          </div>
          <div className="text-sm text-white/70 max-md:w-full max-md:text-xs">{match.mapname}</div>
        </div>

        {/* Teams and Score - Desktop */}
        <div className="hidden md:flex items-center justify-between gap-4 mb-3">
          <div className="flex items-center gap-4 flex-1">
            <div className={`font-semibold truncate flex-1 ${isPlayerTeam1 ? 'text-accent-primary' : 'text-white/70'}`}>
              {match.team1_name}
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <span
                className={`text-xl font-bold ${match.team1_score > match.team2_score ? 'text-accent-primary' : 'text-white/60'}`}>
                {match.team1_score}
              </span>
              <span className="text-white/50">-</span>
              <span
                className={`text-xl font-bold ${match.team2_score > match.team1_score ? 'text-accent-primary' : 'text-white/60'}`}>
                {match.team2_score}
              </span>
            </div>
            <div
              className={`font-semibold truncate flex-1 text-right ${isPlayerTeam2 ? 'text-accent-primary' : 'text-white/70'}`}>
              {match.team2_name}
            </div>
          </div>
        </div>
        ...
        <div className="md:hidden space-y-1.5 mb-3">
          {/* Team 1 */}
          <div className={`flex items-center gap-3 pl-2 py-1.5 border-l-4 ${match.team1_score > match.team2_score
            ? 'border-accent-primary'
            : 'border-transparent'
          }`}>
            <span
              className={`text-lg font-bold min-w-[24px] ${match.team1_score > match.team2_score ? 'text-accent-primary' : 'text-white/60'
              }`}>
              {match.team1_score}
            </span>
            <div
              className={`font-medium text-sm truncate flex-1 ${isPlayerTeam1 ? 'text-accent-primary' : 'text-white/70'
              }`}>
              {match.team1_name}
            </div>
          </div>
          ...
          {/* Team 2 */}
          <div className={`flex items-center gap-3 pl-2 py-1.5 border-l-4 ${match.team2_score > match.team1_score
            ? 'border-accent-primary'
            : 'border-transparent'
          }`}>
            <span
              className={`text-lg font-bold min-w-[24px] ${match.team2_score > match.team1_score ? 'text-accent-primary' : 'text-white/60'
              }`}>
              {match.team2_score}
            </span>
            <div
              className={`font-medium text-sm truncate flex-1 ${isPlayerTeam2 ? 'text-accent-primary' : 'text-white/70'
              }`}>
              {match.team2_name}
            </div>
          </div>
        ...
        {/* Stats */}
        <div className="flex items-center gap-4 text-sm max-md:gap-3 max-md:text-xs">
          <div className="text-white/70">K/D/A: <span
            className="text-white font-medium">{match.kills}/{match.deaths}/{match.assists}</span></div>
          <div className="text-white/70">DMG: <span
    ...
    <div className="bg-gradient-to-br from-[#2b161b] to-[#1a0f12] rounded-[30px] p-6">
      <div className="flex gap-4 mb-6 border-b border-white/10">
        <button
          ...
          className={`pb-3 px-4 font-semibold transition-colors relative ${activeTab === 'statistics'
            ? 'text-accent-primary'
        ...
        <button
          ...
          className={`pb-3 px-4 font-semibold transition-colors relative ${activeTab === 'matchHistory'
            ? 'text-accent-primary'
            : 'text-white/50 hover:text-white/70'
          }`}
        >
